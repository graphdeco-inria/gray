...
class Camera(Widget):
    ...
    def set(self, cam_info: "CameraInfo"):
        ...
        self.right = mat[:3, 0]
        self.right = self.right / np.linalg.norm(self.right)
        self.fov_x = cam_info.fov_x
        self.fov_y = cam_info.fov_y

    def import_client_modules(self):
        global imgui
        from imgui_bundle import imgui

    def set_pose(self, R: np.ndarray, origin: np.ndarray):
        """
        Update the camera pose using a rotation matrix (3x3) and an origin vector (3,).

        Args:
            R (np.ndarray): 3x3 rotation matrix.
            origin (np.ndarray): 3-element position vector.
        """
        ...
        self.right = R[:, 0] / np.linalg.norm(R[:, 0])
        self.up = -R[:, 1] / np.linalg.norm(R[:, 1])    # note the negative to match your convention
        self.forward = R[:, 2] / np.linalg.norm(R[:, 2])
